...
import {
    ...
    isMinDate,
    isValue
} from '../Calendar/shared/propTypes';
import {
    between,
    callIfDefined,
    mergeFunctions
} from '../Calendar/shared/utils';
...
export default class Calendar extends Component {
    get drillDownAvailable() {
        const views = getLimitedViews(
            this.props.minDetail,
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        const { minDate, maxDate, minDetail, maxDetail } = nextProps;
        ...
        if (
            nextView !== prevState.view &&
            !isViewAllowed(prevState.view, minDetail, maxDetail)
        ) {
    ...
    drillUp = () => {
        ...
        this.setState(
            prevState => {
                const nextView = views[views.indexOf(prevState.view) - 1];
                const activeStartDate = getBegin(
                    nextView,
